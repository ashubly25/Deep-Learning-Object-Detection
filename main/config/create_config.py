...
def save_dict(dict, name="squeeze.config"):
    ...
    with open(name, "w") as f:
        ...
        json.dump(dict, f, sort_keys=True, indent=0)
...
def load_dict(path):
    ...
    cfg.ANCHOR_BOX, cfg.N_ANCHORS_HEIGHT, cfg.N_ANCHORS_WIDTH = set_anchors(
        cfg)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Creates config file for squeezeDet training')
    parser.add_argument(
        "--name", help="Name of the config file. DEFAULT: squeeze.config")

...
def read_image_and_gt(img_files, gt_files, config):
    ...
    for img_name, gt_name in zip(img_files, gt_files):
        ...
        for i in range(len(bboxes_per_file)):
            ...
            if aidx == len(config.ANCHOR_BOX):
                dist = np.sum(
                    np.square(bboxes_per_file[i] - config.ANCHOR_BOX), axis=1)
    ...
    Y = np.concatenate((input_mask, box_input,  box_delta_input,
                        labels), axis=-1).astype(np.float32)
...
def read_image_and_gt_with_original(img_files, gt_files, config):
    ...
    imgs = np.zeros((config.BATCH_SIZE, config.IMAGE_HEIGHT,
                     config.IMAGE_WIDTH, config.N_CHANNELS))
    imgs_only_resized = np.zeros(
        (config.BATCH_SIZE, config.IMAGE_HEIGHT, config.IMAGE_WIDTH, config.N_CHANNELS))
...
def generator_from_data_path(img_names, gt_names, config, return_filenames=False, shuffle=False):
   ...
   """
    ...
    assert len(img_names) == len(
        gt_names), "Number of images and ground truths not equal"
    ...
    while 1:
        ...
        for _ in range(nbatches):
            ...
            try:
                ...
                imgs, gts = read_image_and_gt(
                    img_names_batch, gt_names_batch, config)
    ...
    while 1:
        ...
        for _ in range(nbatches):
            ...
            try:
                ...
                imgs, gts, imgs_only_resized = read_image_and_gt_with_original(
                    img_names_batch, gt_names_batch, config)
